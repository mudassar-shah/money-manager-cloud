...
- **Category search on the Categories tab** (added 2026-08-09): the type-to-filter boxes below cover the two *dropdowns*, but the Categories tab's own list had none — with ~79 categories, editing one meant scrolling the whole page. A **Find a category** box now filters both the Expense and Income lists live, matching a category's own name.
  - **A non-matching parent is still drawn, greyed out**, whenever one of its subcategories matches, so a subcategory is never shown stranded without its context. A parent that matches keeps its whole group, as elsewhere (Section 3.5).
  - **Reordering is disabled while a filter is active.** `moveCategory` swaps `order` with the adjacent sibling in the *full* list, so with siblings hidden an arrow would appear to do nothing — it swapped with something off-screen. The arrows are disabled with a "Clear the search to reorder" tooltip rather than left to confuse.
  - The hint reads "**N matches in M categories**", not "showing N of M": because a context parent is drawn too, the row count on screen is legitimately higher than the match count, and the wording must not claim otherwise.
